...
class StreamInferenceSession:
    ...
    def step(self, request: FrameRequest) -> FrameResult:
        ...
        previous_request_cfg_scale = stream_state.get("cfg_scale", missing_cfg_scale) if isinstance(stream_state, dict) else missing_cfg_scale
        if isinstance(stream_state, dict):
            stream_state["cfg_scale"] = float(request.sampler.cfg)
        ...
        try:
            ...
            if isinstance(stream_state, dict):
                if previous_request_cfg_scale is missing_cfg_scale:
                    stream_state.pop("cfg_scale", None)
                else:
                    stream_state["cfg_scale"] = previous_request_cfg_scale

        latency_ms = (time.perf_counter() - t0) * 1000.0

        if out is None:
            out = request.color.convert("RGB")
